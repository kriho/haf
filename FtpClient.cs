...
namespace HAF.Ftp
{
    ...
    public class FtpClient : ObservableObject
    {
        ...
        public MemoryStream DownloadToStream(string remoteFile, long fileSize = 0, ObservableTask task = null) {
            ...
            task?.ReportProgress(0, (int)fileSize, this.GetMessageText(Messages.StartDownloadStream));
            try {
                ...
                using (var response = (FtpWebResponse)ftpRequest.GetResponse()) {
                    ...
                    using (var ftpStream = response.GetResponseStream()) {
                        ...
                        while (task == null || !task.IsCancelled) {
                            ...
                            // report progress
                            if (fileSize > 0) {
                                task?.ReportProgress((int)stream.Position, (int)fileSize);
                            }
                        }
                    }
                }
        ...
        public Task<MemoryStream> DownloadToStreamAsync(string remoteFile, long fileSize = 0, ObservableTask task = null) {
            return Task.Run(() => {
                return this.DownloadToStream(remoteFile, fileSize, task);
            });
        }

        public void DownloadToFile(string remoteFile, string localFile, long fileSize = 0, ObservableTask task = null) {
            ...
            // report initial progress
            task?.ReportProgress(0, (int)fileSize, this.GetMessageText(Messages.StartDownloadFile));
            try {
                // get response from server
                using (var response = (FtpWebResponse)ftpRequest.GetResponse()) {
                    // get response stream
        ...
        public Task DownloadToFileAsync(string remoteFile, string localFile, long fileSize = 0, ObservableTask task = null) {
            return Task.Run(() => {
                this.DownloadToFile(remoteFile, localFile, fileSize, task);
            });
        }

        public bool DownloadToFileVerified(string remoteFile, string localFile, string md5, long fileSize = 0, ObservableTask task = null) {
            this.DownloadToFile(remoteFile, localFile, fileSize, task);
            if (this.Reporter.HasErrors) {
                return false;
            }
        ...
        public Task<bool> DownloadToFileVerifiedAsync(string remoteFile, string localFile, string md5, long fileSize = 0, ObservableTask task = null) {
            return Task.Run(() => {
                return this.DownloadToFileVerified(remoteFile, localFile, md5, fileSize, task);
            });
        }

        public void UploadFile(string remoteFile, string localFile, long fileSize = 0, ObservableTask task = null) {
            ...
            // report initial progress
            task?.ReportProgress(0, (int)fileSize, this.GetMessageText(Messages.StartUploadFile));
            try {
                // get response stream
        ...
        public Task UploadFileAsync(string remoteFile, string localFile, long fileSize = 0, ObservableTask task = null) {
            return Task.Run(() => {
                this.UploadFile(remoteFile, localFile, fileSize, task);
            });
        }

        public bool UploadFileVerified(string remoteFile, string localFile, string md5, long fileSize = 0, ObservableTask task = null) {
            this.UploadFile(remoteFile, localFile, fileSize, task);
            if (this.Reporter.HasErrors) {
                return false;
            }
            using (var stream = this.DownloadToStream(remoteFile, fileSize, task)) {
                if (this.Reporter.HasErrors) {
                    return false;
                }
                return md5 == this.CalculateCRC(stream);
        ...
        public Task<bool> UploadFileVerifiedAsync(string remoteFile, string localFile, string md5, long fileSize = 0, ObservableTask task = null) {
            return Task.Run(() => {
                return this.UploadFileVerified(remoteFile, localFile, md5, fileSize, task);
            });
        }

        public List<string> ListDirectories(string remoteDirectory) {
        ...
        public Task<List<string>> ListDirectoriesAsync(string remoteDirectory) {
            return Task.Run(() => {
                return this.ListDirectories(remoteDirectory);
            });
        }

        public bool DirectoryExists(string remoteDirectoryParent, string remoteDirectory) {
            var directories = this.ListDirectories(remoteDirectoryParent);
            if (this.Reporter.HasErrors) {
                return false;
            }
        ...
        public Task<bool> DirectoryExistsAsync(string remoteDirectoryParent, string remoteDirectory) {
            return Task.Run(() => {
                return this.DirectoryExists(remoteDirectoryParent, remoteDirectory);
            });
        }
        ...
        public Task CreateDirectoryAsync(string remoteDirectory) {
            return Task.Run(() => {
        ...
        public Task RemoveDirectoryAsync(string remoteDirectory) {
            return Task.Run(() => {
                this.RemoveDirectory(remoteDirectory);
            });
        }
        ...
        public Task DeleteFileAsync(string remoteFile) {
            return Task.Run(() => {
                this.DeleteFile(remoteFile);
            });
        }

        public void RenameFile(string remoteFile, string newRemoteFile) {
        ...
        public Task RenameFileAsync(string remoteFile, string newRemoteFile) {
            return Task.Run(() => {
                this.RenameFile(remoteFile, newRemoteFile);
            });
        }
    }
}
